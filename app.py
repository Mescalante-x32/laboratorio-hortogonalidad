...
# =========================================================
# MÓDULO 13: ONDA COMPLETA CON FILTRO L-C
# =========================================================
elif tema == "13. Onda Completa con Filtro L-C":
    st.header("Módulo 13: Rectificador con Filtro L-C (Entrada por Inductor)")
    st.write("Análisis de filtrado de segundo orden y suavizado de corriente de línea.")

    with st.sidebar:
        st.subheader("Parámetros del Filtro L-C")
        Vm_13 = st.number_input("Voltaje Pico Fuente [V]", value=170.0, key="vm13")
        f_13 = st.number_input("Frecuencia [Hz]", value=60.0)
        
        st.markdown("---")
        Lf_mH = st.number_input("Inductancia del Filtro (Lf) [mH]", value=100.0, step=10.0)
        Cf_uF = st.number_input("Capacitancia del Filtro (Cf) [µF]", value=470.0, step=50.0)
        R_13 = st.number_input("Resistencia de Carga [Ω]", value=50.0, min_value=1.0)
        
        num_ciclos_13 = st.slider("Ciclos a simular", 2, 30, 10, key="c13")

    # --- Configuración de Simulación Numérica ---
    w = 2 * np.pi * f_13
    Lf = Lf_mH / 1000
    Cf = Cf_uF * 1e-6
    
    puntos = num_ciclos_13 * 1000
    theta = np.linspace(0, 2 * np.pi * num_ciclos_13, puntos)
    dt = (theta[1] - theta[0]) / w
    
    v_s_abs = np.abs(Vm_13 * np.sin(theta))
    v_out = np.zeros(puntos) # Voltaje en la carga (sobre C)
    i_f = np.zeros(puntos)   # Corriente a través de Lf
    ...
    for i in range(1, puntos):
        ...
        # El puente de diodos conduce si el voltaje de entrada es mayor al del capacitor
        # O si hay corriente circulando por el inductor de filtro (continuidad)
        if v_s_abs[i] > v_c_val or i_l_val > 1e-4:
            # diL/dt = (V_rect - V_cap) / Lf
            di_l = ((v_s_abs[i] - v_c_val) / Lf) * dt
            i_l_val += di_l
            if i_l_val < 0: i_l_val = 0 # El puente no permite corriente inversa
            
            # dvC/dt = (i_L - i_load) / Cf
            dv_c = ((i_l_val - i_load) / Cf) * dt
            v_c_val += dv_c
        else:
            # Ambos diodos bloqueados (raro en filtro LC, pero posible con R baja)
            i_l_val = 0
            dv_c = (-i_load / Cf) * dt
            v_c_val += dv_c
    ...
    v_dc_avg = np.mean(v_out[ultimo_ciclo:])
    v_ripple_lc = np.max(v_out[ultimo_ciclo:]) - np.min(v_out[ultimo_ciclo:])
    
    # Condición de Inductancia Crítica Lc
    # Lc > R / (3 * w) para rectificador de onda completa
    L_critica = R_13 / (3 * w) * 1000 # en mH

    col1, col2, col3 = st.columns(3)
    col1.metric("Vdc Promedio", f"{v_dc_avg:.2f} V")
    col2.metric("Rizado de Voltaje", f"{v_ripple_lc:.2f} V")
    col3.metric("L Crítica Sugerida", f"{L_critica:.1f} mH")

    # --- Gráficas ---
    x_ciclos = theta / (2 * np.pi)
    fig14, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)
    
    ax1.plot(x_ciclos, v_s_abs, 'gray', ls='--', alpha=0.2, label="|v_fuente|")
    ax1.plot(x_ciclos, v_out, 'b', lw=2, label="v_carga (Filtrado LC)")
    ax1.set_ylabel("Voltaje [V]"); ax1.legend(); ax1.grid(True)
    ...
    st.success(f"""
    **Lección para el alumno:**
    1. **Conducción Continua:** Observe que con Lf = {Lf_mH} mH, la corriente del inductor es mucho más constante que los pulsos del filtro C.
    2. **Inductancia Crítica:** Si Lf < {L_critica:.1f} mH, la corriente caerá a cero en cada ciclo (conducción discontinua), aumentando el rizado.
    3. **Regulación:** El filtro L-C ofrece una mejor regulación de carga y estresa mucho menos a los diodos y a la red eléctrica.
    """)
